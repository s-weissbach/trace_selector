    ...
    def stdin(self):
        while (line := sys.stdin.readline().strip("\n")) != "":
            ...
            match args[0].lower():
                case "open":
                    if len(args) != 2:
                        print("[TRACESELECTOR] [SYNTAX_ERROR] open [path]")
                        continue
                    path = args[1].strip("'").strip('"')
                    if not pathlib.Path(args[1]).exists():
                        print(f"[TRACESELECTOR] [FILE_NOT_EXISTS_ERROR] File {path} does not exists")
                        continue
                    print(f"[TRACESELECTOR] [OPEN_FILE] Opening file {path}")
                    self.signal.emit(path)
                case _:
                    print(f"[TRACESELECTOR] [UNKOWN_CMD_ERROR] Unknown command '{args[0]}'")
        print("[TRACESELECTOR] [STOPED_API]")